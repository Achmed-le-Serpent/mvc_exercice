...
RSpec.describe Item, type: :model do
  ...
  describe 'price' do
    context "when the item dosen't have a discount" do
  ...
  describe 'average_price' do
    ...
    context 'average_price is nil when database is empty' do
      it { expect(subject.average_price).to eq(nil) }
    end

    context 'average_price is a float' do
      let!(:item) { create(:item) }
      it { expect(subject.average_price.class).to be(Float) }
      it { expect(subject.average_price).to eq(item.price) }
    end
  end
